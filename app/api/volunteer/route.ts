...
export async function POST(request: NextRequest) {
  ...
  const errorResponse = await submitToWeb3Forms(
    {
      ...
      age: data.age,
      volunteer_track: data.track,
      availability: data.availability,
      message: data.message ?? "Not provided",
    },
    `[Shree Prathishthan] Volunteer Application — ${data.track}`,
    data.name,
    "Volunteer API"
  );
  ...
  await saveSubmission("volunteer", data as unknown as Record<string, unknown>);

  return NextResponse.json(
    { success: true, message: "Your application has been received. Our team will contact you shortly." },
    { status: 200 }
  );
}
