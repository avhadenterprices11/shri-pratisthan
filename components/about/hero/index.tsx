...
export default function AboutHero() {
  ...
  return (
    <section 
      ref={containerRef}
      className="relative min-h-[50vh] flex flex-col justify-center items-center py-24 px-6 overflow-hidden md:px-12 text-center"
    >
      ...
      <div className="w-full max-w-4xl relative z-10">
        <div className="inline-flex items-center gap-2 mb-4 bg-saffron/10 text-saffron font-bold text-xs uppercase tracking-widest px-4 py-1.5 rounded-full border border-saffron/20">
          <span>About Shree Prathishthan</span>
        </div>
        ...
        <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold text-foreground leading-[1.05] tracking-tight mb-6 font-heading">
          <div className="overflow-hidden">
            <span className="block reveal-line">Our Story &</span>
          </div>
          <div className="overflow-hidden">
            <span className="block reveal-line text-saffron text-outline-festive">Core Foundations.</span>
          </div>
        </h1>

        <p className="hero-subtitle text-base sm:text-lg text-slate-grey max-w-2xl mx-auto leading-relaxed">
          Tracing the history, vision, and people driving our dual commitment to cultural preservation and active community service in Maharashtra.
        </p>
      </div>
    </section>
  );
