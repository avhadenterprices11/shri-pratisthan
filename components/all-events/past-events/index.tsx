  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-10 sm:py-16">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-2.5 sm:space-y-3 mb-8 sm:mb-12">
        ...
        <h2 className="text-2xl sm:text-4xl md:text-5xl font-normal font-heading text-neutral-900 tracking-tight uppercase">
          PAST EVENTS &amp; <span className="text-saffron font-heading">COMMUNITY IMPACT</span>
        </h2>
        <p className="text-xs sm:text-base text-slate-grey font-normal leading-[1.7] sm:leading-[1.75] font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 sm:gap-6 md:gap-8">
        {pastEvents.map((event) => (
          <div
            key={event.id}
            className="bg-white text-neutral-900 rounded-2xl sm:rounded-3xl overflow-hidden border border-neutral-200 shadow-sm hover:shadow-xl hover:border-saffron/40 flex flex-col justify-between group transition-all duration-500"
          >
            ...
            <div className="p-4 sm:p-6 space-y-3 sm:space-y-4 flex-grow flex flex-col justify-between">
              <div className="space-y-1.5 sm:space-y-2">
                <h4 className="text-base sm:text-lg font-normal font-heading text-neutral-900 group-hover:text-saffron transition-colors uppercase">
                  {event.title}
                </h4>
              ...
              <div className="grid grid-cols-2 gap-2 pt-2 border-t border-neutral-100 text-xs">
                {event.metrics.slice(0, 2).map((m, idx) => (
                  <div key={idx} className="p-2 sm:p-2.5 bg-neutral-50 rounded-xl border border-neutral-200/80 text-center">
                    <span className="font-normal text-saffron block font-heading">{m.value}</span>
                    <span className="text-[9px] sm:text-[10px] text-neutral-500 font-normal block truncate font-sans">{m.label}</span>
                  </div>
                ))}
              </div>
              ...
              <Link
                ...
                className="w-full py-2 sm:py-2.5 bg-neutral-900 hover:bg-neutral-800 text-white border border-neutral-800 rounded-xl text-[10px] sm:text-xs font-bold uppercase tracking-[0.16em] text-center transition-all flex items-center justify-center gap-1.5 shadow-sm font-sans"
              >
